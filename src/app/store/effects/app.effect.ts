    ...
    updateCode$ = createEffect(() => {
        return this.actions$.pipe(
            ...
            exhaustMap((action) => this.appRequest.updateCode(action.projectId, action.code).pipe(
                mergeMap(data => [replaceCode({ code: data }), updateCodeSuccess({ code: data })]),
                catchError(err => of(updateCodeError({ err, code: action.code }), requestError({ message: err.error, error: err })))
    ...
    createChat$ = createEffect(() => {
        return this.actions$.pipe(
            ...
            exhaustMap((action) => this.appRequest.createChat(action.projectId, action.chat).pipe(
                mergeMap((data) => [addChat({ chat: data }), createChatSuccess({ chat: data })]),
                catchError(err => of(createChatError({ err, chat: action.chat }), requestError({ message: err.error, error: err })))
            )),
        );
    });
    ...
    updateChat$ = createEffect(() => {
        return this.actions$.pipe(
            ...
            exhaustMap((action) => this.appRequest.updateChat(action.projectId, action.chat).pipe(
                mergeMap((data) => [replaceChat({ chat: data }), updateChatSuccess({ chat: data })]),
                catchError(err => of(updateChatError({ err, chat: action.chat }), requestError({ message: err.error, error: err })))
            )),
        );
    });

    streamChat$ = createEffect(() => {
        return this.actions$.pipe(
            ofType(streamChat),
            exhaustMap((action) => this.appRequest.streamChat(action.projectId, action.chatId).pipe(
                map(data => chunkChat({ chatId: action.chatId, chunk: data })),
                catchError(err => of(requestError({ message: err.error, error: err })))
            )),
        );
    });
